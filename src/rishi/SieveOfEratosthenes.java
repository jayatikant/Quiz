...
/**
 * Complexity of this program is O(nlog(logn)). Loop in method applySieve() runs
 * for square root of total numbers thus making loop complexity as log(logn).
 * That loop internally calls
 * markAllMultiplesOfNumberAsNonPrimeIfNumberPrime(int number) method which runs
 * a loop of n times. Thus making program complexity as a whole O(nlog(logn))
 */
//naming  4.5
	// sc :(
...
//algorithm fulfillment  4
	//array initialized even though input may not be valid
	//finding total no of primes instead of largest prime
public class SieveOfEratosthenes {
	...
	private static void displayInputRangeExceededMessageIfInputIsNotValid() {
		System.out
				.println("Input range exceeded!! Must be less than 10 million!");
	}
	...
	private static void markAllMultiplesOfNumberAsNonPrimeIfNumberPrime(
			int number) {
		if (isPrime(number)) {
	...
	private static void printTotalNumberOfPrimes() {
		System.out.println("Total Number of Primes in range 2 to "
				+ totalNumbers + " are:" + totalPrimes);
	}
